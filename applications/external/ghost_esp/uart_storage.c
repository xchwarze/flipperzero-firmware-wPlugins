...
void uart_storage_rx_callback(uint8_t* buf, size_t len, void* context) {
    ...
    if(!app->storageContext->HasOpenedFile || !app->storageContext->current_file ||
    ...
    while(written < len) {
        ...
        size_t chunk_size = (remaining < PCAP_WRITE_CHUNK_SIZE) ? remaining :
                                                                  PCAP_WRITE_CHUNK_SIZE;

        uint16_t bytes_written =
            storage_file_write(app->storageContext->current_file, buf + written, chunk_size);
