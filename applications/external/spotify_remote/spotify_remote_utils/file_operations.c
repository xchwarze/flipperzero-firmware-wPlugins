...
void save_ssid(const char* ssid) {
    ...
    bool file_open_ret = storage_file_open(
        file, EXT_PATH("apps/Examples/ssid.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS);
...
void save_password(const char* ssid) {
    // We need a storage struct (gain access to the filesystem API )
    Storage* storage = furi_record_open(RECORD_STORAGE);
    ...
    bool file_open_ret = storage_file_open(
        file, EXT_PATH("apps/Examples/pass.txt"), FSAM_WRITE, FSOM_CREATE_ALWAYS);
