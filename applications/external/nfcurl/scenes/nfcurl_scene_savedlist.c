...
static void
    nfcurl_scene_savedlist_button_menu_callback(void* context, int32_t index, InputType type) {
...
void nfcurl_scene_savedlist_on_enter(void* context) {
    ...
    size_t split;
    size_t index = 0;
    while(stream_read_line(urlstream, line) &&
          index < 100) { // TODO: Don't have a hardcoded maximum
        ...
        furi_string_set_n(
            app->urlpairs[index].url, line, split + 2, furi_string_size(line) - split - 2);
