...
static bool nfcurl_save_url(NfcUrlApp* app) {
    ...
    if(!file_stream_open(
           urlstream, furi_string_get_cstr(outpath), FSAM_READ_WRITE, FSOM_OPEN_ALWAYS)) {
        FURI_LOG_E("nfcurl", "Error opening url save file");
        success = false;
    }

    if(!stream_insert_format(
           urlstream,
           "%s:%c%s\n",
...
void nfcurl_scene_save_on_enter(void* context) {
    ...
    text_input_set_result_callback(
        app->text_input, nfcurl_scene_save_text_callback, app, app->text_buffer, 200, true);
...
bool nfcurl_scene_save_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == NfcUrlCustomEventTypeTextEditDone) {
            ...
            popup_set_header(
                app->popup, (success ? "Saved" : "Save Failed"), 95, 26, AlignCenter, AlignBottom);
