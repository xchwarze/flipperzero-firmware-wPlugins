                ...
                switch(input.key) {
                    ...
                    case InputKeyDown:
                        if (l->menu_state == MenuMain) {
                            ...
                            if ((l->selected_key == 3 || l->selected_key == 5 || l->selected_key == 4) && 
                                l->counts[l->selected_key][l->active_column] < 5) {
                                l->counts[l->selected_key][l->active_column]++;
                            } else if (l->selected_key != 3 && l->selected_key != 5 && l->selected_key != 4 && 
                    ...
                    case InputKeyLeft:
                        if (l->menu_state == MenuShow) {
                            l->active_column = (l->active_column - 1 + (l->selected_key == 4 ? TOY48_COLUMNS : OTHER_COLUMNS)) % (l->selected_key == 4 ? TOY48_COLUMNS : OTHER_COLUMNS);
                        }
                        break;
                    case InputKeyRight:
                        if (l->menu_state == MenuShow) {
                            l->active_column = (l->active_column + 1) % (l->selected_key == 4 ? TOY48_COLUMNS : OTHER_COLUMNS);
                        }
                        break;
                    case InputKeyOk:
                        if (l->menu_state == MenuMain) {
                            if (l->selected_menu == 0) l->menu_state = MenuConfig;
                            else if (l->selected_menu == 1) l->menu_state = MenuShow;
                            else if (l->selected_menu == 2) l->menu_state = MenuAbout;
                        } else if (l->menu_state == MenuConfig) {
                            const char* keys[MAX_KEYS] = {"HU66", "HU92", "HU83", "K5", "TOY48", "FO38"}; 
